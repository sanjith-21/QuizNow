...
<body>
    ...
    // echo $qid;
    // echo $uid;
    $result33 = $con->query("SELECT distinct(status) FROM qattempt where sid='$uid' and qid='$qid'");
    // echo "hello";
    // echo $result33->num_rows;
    while ($rows1 = $result33->fetch_assoc()) {
        // echo "dvcsd";
            ...
            while ($rows = $result->fetch_assoc()) {
                ...
                if ($rows["qdate"] == $currentdate) {
                    ...
                    if ($rows["stime"] <= $currenttime) {
                        ...
                        $con->query("update qattempt set status='Started' where qid='$qid' and sid='$uid'");
                        $status = "ACTIVE";
                        if ($rows["etime"] <= $currenttime) {
                            $check1 = $con->query("update qlist set status='COMPLETED' where qid='$qid'");
                            $con->query("update qattempt set status='Not Started' where qid='$qid' and sid='$uid'");
                            $status = "COMPLETED";
                        }
                    }
        ...
        <fieldset style="background-color: #0ead88;">
            <table style="width:100%" cellspacing="10">
                ...
                <tr>
                    <td>Quiz Date</td>
                    <td> :
                         <input form="form1" type="date" name="qdate" value="<?php echo $qdate; ?>"
                            readonly>
                    </td>
                </tr>
                <tr>
            ...
            while ($rows = $result1->fetch_assoc()) {
                ...
                <div id="questions">
                    <div class="question center">
                        ...
                        <input form="form1" type="hidden" name="question[<?php echo ($i + 1); ?>]"
                            value="<?php echo $rows["questions"] ?>" />
                    </div>
                    <div class="answer center">
                        ...
                        <ol type="A">
                            ...
                            for ($j = 1; $j <= 4; $j++) {
                                ?>
                                <li><input form="form1" type="<?php echo $rows["type"]; ?>"
                                        name="option[<?php echo ($i + 1); ?>][]" value="<?php echo $rows["option$j"]; ?>" />
    ...
    <script>
        ...
        // alert(document.form1.qdate.value);
