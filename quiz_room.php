...
<body>
        ...
        <fieldset style="background-color: #0ead88;">
            <table style="width:100%" cellspacing="10">
                ...
                <tr>
                    <td>Start Time</td>
                    <?php if (isset($_REQUEST["edit"])) { ?>
                        <td> : <input form="form1" type="time" name="stime" value="<?php echo $stime; ?>"
                                onchange="date1()"></td>
                    <?php } else { ?>
                        <td> : <input form="form1" type="time" name="stime" value="<?php echo $stime; ?>" onchange="date1()"
                                readonly></td>
                    <?php } ?>
                </tr>
                <tr>
                    <td>End Time</td>
                    <?php if (isset($_REQUEST["edit"])) { ?>
                        <td> : <input form="form1" type="time" name="etime" value="<?php echo $etime; ?>"
                                onchange="date1()"></td>
                    <?php } else { ?>
                        <td> : <input form="form1" type="time" name="etime" value="<?php echo $etime; ?>" onchange="date1()"
                                readonly></td>
                    <?php } ?>
                </tr>
                <tr>
                    ...
                    if (isset($_REQUEST["edit"])){
                        if ($shuffle == "yes") {
                            ?>
                            <td> : <input form="form1" type="radio" name="shuffle" value="yes" checked />YES <input form="form1"
                                    type="radio" name="shuffle" value="no" />NO</td>
                            <?php
                        } else {
                            ?>
                            <td> : <input form="form1" type="radio" name="shuffle" value="yes" />YES <input form="form1"
                                    type="radio" name="shuffle" value="no" checked />NO</td>
                            <?php
                        }
                    } else {
                        ?>
                    ...
                    <td id="student_list1" colspan="2">
                        <div class="scroll" style="height:200px;">
                            <table cellspacing="20" class="tableview">
                                ...
                                <?php
                                $result3 = $con->query("select distinct(sid),status,fname from qattempt inner join login on sid=uid where qid='$qid'");
                                ...
                                while ($rows = $result3->fetch_array()) {
                                    ...
                                    <tr>
                                        ...
                                        if ($rows["status"] == "Completed") {
                                            $result4 = $con->query("select * from qmarks where qid='$qid' and sid='" . $rows["sid"] . "' ");
                                            while ($rows1 = $result4->fetch_assoc()) {
                                                ?>
                                                <td>
                                                    <?php echo $rows1["marks"]; ?>
                                                </td>
                                                <?php
                                            }
                                        } else {
                ...
                <tr>
                    <?php if (isset($_REQUEST["edit"])) { ?>
                        <td colspan="2" id="update">
                            <button style="width:30%;" class="logoutbutton"
                                onclick="location.href='quiz_room.php?qid=<?php echo $qid; ?>'">BACK</button>
                            &ensp;&ensp;&ensp;&ensp;&ensp;
                            <button style="width:30%;" class="logoutbutton"
                                onclick="update(<?php echo $qid; ?>)">UPDATE</button>
                        </td>
                    <?php } else { ?>
            ...
            while ($rows = $result1->fetch_assoc()) {
                ...
                <div id="questions">
                    <div class="question center">
                        ...
                        <br>
                        <?php if (isset($_REQUEST["edit"])) { ?>
                            <textarea rows="3" cols="50" form="form1"
                                name="question[<?php echo ($i + 1); ?>]"><?php echo $rows["questions"]; ?></textarea>
                        <?php } else { ?>
                            <textarea rows="3" cols="50" form="form1" name="question[<?php echo ($i + 1); ?>]"
                                readonly><?php echo $rows["questions"]; ?></textarea>
                        <?php } ?>
                    </div>
                    <div class="answer center">
                        <input form="form1" type="hidden" name="type[<?php echo ($i + 1); ?>]"
                        ...
                        for ($j = 1; $j <= 4; $j++) {
                            ...
                            for ($k = 0; $k < count($answer); $k++) {
                                if ($rows["option$j"] == $answer[$k]) {
                                    ?>
                                    <input form="form1" type="<?php echo $rows["type"]; ?>" name="option[<?php echo ($i + 1); ?>][]"
                                        value="<?php echo $rows["option$j"]; ?>" checked />
                                    <?php
                                    $flag = 1;
                                    break;
                                }
                            ...
                            if ($flag == 0) {
                                ?>
                                <input form="form1" type="<?php echo $rows["type"]; ?>" name="option[<?php echo ($i + 1); ?>][]"
                                    value="<?php echo $rows["option$j"]; ?>" />
                                <?php
                            }
                            if (isset($_REQUEST["edit"])) { ?>
                                <input form="form1" class="answerinput" type="text" name="answer[<?php echo ($i + 1); ?>][]"
                                    value="<?php echo $rows["option$j"]; ?>" /><br>
                            <?php } else { ?>
                                <input form="form1" class="answerinput" type="text" name="answer[<?php echo ($i + 1); ?>][]"
                                    value="<?php echo $rows["option$j"]; ?>" readonly /><br>
                            <?php }
                        }
                        ?>
                    </div>
    ...
    <div class="tabcontent" id="tabcontent1">
        <fieldset class="scroll">
            ...
            <?php
            $i1 = 0;
            $result2 = $con->query("select * from qquestions where qid='$qid'");
